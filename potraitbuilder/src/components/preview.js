  ...
  const [volumeName, imageFolder] = student.relativePath.split("/");
  ...
  async function uploadToBackend(file, teacher, grade, homeRoom, firstName, lastName, imageFolder, fileName, volumeName) {
...
async function handleUpload() {
  ...
  for (const [gradeName, students] of Object.entries(gradeGroups)) {
    for (const rawStudent of students) {
       const student = enrichStudentData(rawStudent);
      const teacherName = student.teacherName || "UnknownTeacher";
      const homeRoom = student.homeRoom || "000";

      const uploadPromise = uploadToBackend(
        student.file,
        teacherName,
        gradeName,
        homeRoom,
        ...
        console.log(`✅ Uploaded ${student.file.name}`);
      }).catch((err) => {
